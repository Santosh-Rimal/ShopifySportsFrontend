...
const HomePage = ({ setEmail }) => {
  ...
  useEffect(() => {
    ...
    const fetchCustomerReviews = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/ratingReview/?limit=5`);
        const data = await response.json();
              ...
              <div key={product.id} className="collection-card bg-white shadow-md rounded-lg p-4 flex flex-col relative">
                ...
                <div className='flex gap-10'>
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div>
                      <p className="text-[16px] font-semibold text-green-600">${discountPrice}</p>
                      {product.discount > 0 && (
                ...
                <p className="text-gray-600 h-20 my-4 break-all">{product.description}</p>
                <Link 
      ...
      <section className="featured-collections pb-20 bg-gray-100">
        ...
        <div className="collections-grid grid gap-4 mx-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 max-w-screen-xl">
          {featuredCollections.map(collection => {
            const discountPrice = (collection.price * (1 - collection.discount / 100)).toFixed(2);
            return (
              <div key={collection.id} className="collection-card bg-white shadow-md rounded-lg p-4 flex flex-col relative">
                ...
                <p className="text-gray-600 h-20 my-4 break-all">{collection.description}</p>
                <Link 
        ...
        <div className="reviews-grid grid gap-4 mx-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 max-w-screen-xl">
          {customerReviews.slice(0, 4).map(review => (
            <div key={review.id} className="review-card bg-white shadow-md rounded-lg p-4 flex flex-col items-center">
              <p className="text-gray-800 text-center h-36">"{review.review}"</p>
              <span className="text-gray-600 mt-2 inline-block">- {review.user.name}</span>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className='flex gap-10 mx-8'>
          ...
          >
            ...
            <div className="offer-details relative z-10 py-8 px-4 rounded-lg">
              ...
              <p className="text-lg">Explore our special offers now!</p>
            </div>
          </div>
          <div className="offer-card relative text-white text-center py-20 rounded-lg shadow-md" 
            style={{ 
          ...
          >
            ...
            <div className="offer-details relative z-10 py-8 px-4 rounded-lg">
              ...
              <p className="text-lg">Check out our latest collections</p>
            </div>
          </div>
        </div>
      </section>
